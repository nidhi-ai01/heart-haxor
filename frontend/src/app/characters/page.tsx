...
export default function CharactersPage() {
  ...
  useEffect(() => {
    setFilteredCharacters(
      characters.filter((char) => char.role.toLowerCase() === selectedRole.toLowerCase())
    );
      ...
      <section className="app-panel-strong rounded-[2rem] p-6 sm:p-8">
        <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <h1 className="mt-5 text-4xl font-extrabold tracking-[-0.04em] text-slate-900 dark:text-slate-100">
              Choose your companion
            </h1>
            <p className="mt-3 max-w-2xl text-sm leading-7 text-slate-600 dark:text-slate-400">
              Every character below matches the role you selected, while keeping the same polished
              UI system across discovery and chat.
            </p>
          </div>
        ...
        >
          {filteredCharacters.map((character) => (
            <article
              ...
              className={clsx(
                "cursor-pointer rounded-[2rem] border border-white/50 bg-white/72 p-4 shadow-[0_20px_50px_rgba(15,23,42,0.08)] transition hover:-translate-y-1 hover:shadow-[0_24px_60px_rgba(15,23,42,0.12)] dark:border-white/10 dark:bg-[rgba(8,17,31,0.74)]",
            ...
            >
              <div className={clsx("relative overflow-hidden rounded-[1.5rem]", viewMode === "grid" ? "h-72" : "h-36 w-full sm:w-44")}>
                <img
                  ...
                  onError={(e) => {
                    e.currentTarget.src = `https://placehold.co/400x400/e2e8f0/0f172a?text=${character.name[0]}`;
                  }}
                />
                <button
                  onClick={(e) => {
